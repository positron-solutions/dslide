...
:END:
See the ~dslide-mode-map~ but basically, arrow keys.  Press the right arrow key.
- → ~dslide-presentation-forward~
- ← ~dslide-presentation-backward~
- ↑ ~dslide-presentation-start~ (default secondary action shows contents)
...
#+end_src
#+attr_dslide: final step-backward
#+begin_src elisp :results none
